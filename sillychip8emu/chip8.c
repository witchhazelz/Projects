/*core of chip8 interpreter...im gonna cry how am i gonna finish this in an hour*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdint.h>
...
static const uint8_t hfont[] = {
    /* '0' */ 0x7C, 0x82, 0x82, 0x82, 0x82, 0x82, 0x82, 0x 82, 0x7C, 0x00,
    ...
    switch (opcode & 0xF000) {
        case 0x0000:
            if (opcode == 0x00E0) {
                ...
                /* SYS: ff there's a hook, call it otherwise treat it as a no-op */
                if (c8_sys_hook) {
        ...
        case 0xD000: {
            /* DRW Vx, Vy, nibble */
            int p, q, byte, bit;
            C8.V[0xF] = 0; // clear collision flag
            for (q = 0; q < nibble; q++) {
                for (p = 0; p < 8; p++) {
                    if (C8.RAM[C8.I + q] & (0x80 >> p)) {
                        int x_pos = (C8.V[x] + p) & 0x3F; // wraparound
                        int y_pos = (C8.V[y] + q) & 0x1F; // wraparound
                        byte = y_pos * 64 + x_pos;
                        bit = 1 << (byte & 0x07);
                        byte >>= 3;
                        if (pixels[byte] & bit) {
                        ...
                        pixels[byte] ^= bit; // draw the pixel
                    }
                }
            }
            screen_updated = 1;
...
int c8_ended() {
    return borked || (C8.RAM[C8.PC] == 0x00FD);
}

int c8_waitkey() {
    return (C8.RAM[C8.PC] & 0xF0FF) ==  0xF00A;
}
...
/*took an hour and 10 minutes so not bad ig*/