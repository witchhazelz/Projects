 ...
 *
 * ![toc]
 ## I am using Wernseys chip8 repository as a reference. Credits go toward this repository linked here: https://github.com/wernsey/chip8/blob/master/chip8.h
 * ## Introduction
 *
 ...
 *     changes.
 * * Tell the interpreter about every 60Hz timer tick; see `c8_60hz_tick()`.
 ...
 * are handled.
 *
 * Detailed explanations of the issues can be found in [langhoff][].
 *
 ...
 * * `QUIRKS_SHIFT` - If it is not set, `Vy` is copied into `Vx` when performing the `8xy6` and ```c
 * `8xyE` shift instructions.
 ...
 *
 * It basically just searches for the last non-zero byte in RAM.
 */
uint16_t c8_prog_size();
 ...
 *
 * Returns 1 if the pixel is set - i.e. it should be drawn in the foreground color.
 *
 * Returns 0 if the pixel is cleared - i.e. it should be drawn in the background color.
 */
int c8_get_pixel(int x, int y);
 ...
 * that needs to be `free()`d afterwards.
 *
 ...
 *
 * It has the same semantics as `printf()` and
